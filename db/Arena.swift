...
public class Arena {
    private var alloc_ptr_: UnsafePointer<UInt8>?
    private var alloc_bytes_remaining_: Int = 0
    private var blocks_: ContiguousArray<UnsafePointer<UInt8>> = []
    private let memory_usage_: Atomic<Int> = Atomic<Int>(0)

    private func AllocateFallback(_ bytes: Int) -> UnsafePointer<UInt8> {
        ...
        var result: UnsafePointer<UInt8> = alloc_ptr_!
        alloc_ptr_! += bytes
        alloc_bytes_remaining_ -= bytes
        return result
    }

    private func AllocateNewBlock(_ block_bytes: Int) -> UnsafePointer<UInt8> {
        var results: BytesStorage = BytesStorage(block_bytes)
        blocks_.append(results.pointer)
        memory_usage_.add(block_bytes + MemoryLayout<UnsafePointer<UInt8>>.stride, ordering: .relaxed)
        return results.pointer
    }

    public func Allocate(_ bytes: Int) -> UnsafePointer<UInt8> {
    ...
    public func AllocateAligned(_ bytes: Int) -> UnsafePointer<UInt8> {
        let align: Int = MemoryLayout<UnsafeRawPointer>.stride
