...
public class WriteBatch {
    fileprivate var rep_: BytesStorage = BytesStorage(kHeader)
    ...
    public func Iterate<T: Handler>(_ handler: inout T) -> Status {
        ...
        while !input.empty() {
            found += 1
            let tag: ValueType = ValueType(rawValue: input[0])!
...
public class WriteBatchInternal {
    ...
    public static func SetContents(_ batch: WriteBatch, _ contents: Slice) {
        ...
        batch.rep_ = BytesStorage(contents)
    }

    public static func InsertInto(_ batch: WriteBatch, _ memtable: MemTable) -> Status {
        var inserter: MemTableInserter = MemTableInserter(WriteBatchInternal.Sequence(batch), memtable)
    ...
    public static func Append(_ dst: WriteBatch, _ src: WriteBatch) {
        ...
        dst.rep_.append(src.rep_.pointer + kHeader, src.rep_.count - kHeader)
    }
}
